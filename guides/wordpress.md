...
*SPLOT stick shift by Alan Levine based on [Stick Shift [Transportation]](https://flickr.com/photos/trustypics/42866781801) flickr image by [trustypics](https://flickr.com/people/trustypics) shared under a [Creative Commons (BY-NC) license](https://creativecommons.org/licenses/by-nc/2.0/) modified by adding labels "install" and "update" to gear shift, shared too as CC BY-NC*


Don't be daunted by the word "manual" -- think of it like driving a car with a manual transmission. Once you have mastered the stick shift and the clutch, it becomes second nature. And then you feel more in charge of the machine.
...
You are all done! Take that SPLOT out for a test drive.


### Updating a Manually Installed SPLOT

I am regularly updating SPLOTs to address bugs, issues, but more often to add features. I pledge to never add something that will break a previously set up site, but also keep in mind that I am not a professional programmer! You should then know how to update your SPLOT theme to a new version.
...
Then visit the Github home for your SPLOT and look for what it lists as the current version. If the version on GitHub is a higher number, you may want to update your SPLOT. It is not urgent to frequently update, but is worth checking, maybe every few months or yearly.

How complex is it to manually update? Not complex at all! 

You update a SPLOT theme the exact way you originally installed it. Just download the `.zip` file for new version, and then in your dashboard, go to **Themes** (under **Appearence**). then click **Add Theme** and finally **Upload Theme**. Select the new `.zip` version of your SPLOT theme. Once uploaded you will be asked to confirm updating the theme with the newer version.  
...
If all this manual download/upload of `.zip` files sounds like a hassle, then allow me, like a car salesman, to offer you a one slick alternative... and it can even be set up to automatically update your SPLOT themes. If this sounds too good to pass up, then I recommend trying the [WP Pusher plugin](https://wppusher.com/) which makes it easy to install themes and plugins that are published in GitHub.
...
Then look under the **WP Pusher** menu in your Wordpress Dashboard, and then click the **GitHub** tab. Next, click the **Obtain a GitHub Token** button to get an authentication token. Copy the one that is generated, paste into the field for it, and finally, click **Save GitHub** Token.
...
In the form that appears, under **Theme Repository**, enter `cogdog/truwriter`. If you check,the the option for **Push-to-Deploy** your SPLOT theme will be updated automatically when there is a new version in GitHub. Otherwise, you can choose to use the Update button at your leisure.
...
## Updating Reclaim Hosting's Custom Installed SPLOTs

This applies only to those who previously set up TRU Collector or TRU Writer sites with the installers offered to [Reclaim Hosting](https://reclaimhosting.com/) clients as Featured Applications... if you are creating a *new* SPLOT site [please use intructions above](#manual-installation).

![](reclaim-featured-apps.jpg "")

I deeply appreciate the support of Reclaim Hosting in creating these custom application installers that not only set up SPLOT sites, but also added suggested plugins and provided sample starting content. It also enabled periodic automatic theme updates (though the frequency of update lagged far behind the GitHub versions).
...
Do not worry! Because of this change from Reclaim Hosting, nothing will happen to your SPLOT. But you should know how to update a SPLOT theme yourself.

### How To Identify a Featured App SPLOT

How would you know if any of your sites where installed with one of these apps? Step into my office...
...
A typical WordPress app will be listed displaying the regular blue W of the WordPress icon, but a TRU Collector or TRU Writer site installed as a custom application will have a black and white icon featuring the name of the SPLOT. 

![](reclaim-apps-diff.jpg "")

Also, a standard WordPress site will list a link to `wordpress.org/support` which is not shown on a custom application install. If you see any of your SPLOTs identified by a Featured App icon, here is how you can update at any time.
...
**But you have to make one adjustment first**. For say the TRU Writer theme, WordPress is expecting the theme stored in a directory named `truwriter` and unfortunately GitHub provides it in a directory named `truwriter-master` downloaded as `truwriter-master.zip`. 

This is a zip "dance" you will have to do to make the updated theme apply to your SPLOT:
...
You will know the upload is correct if notifies you the theme is already installed. 

![](uploaded-update.jpg "")

This is the screen you want to see, so click **Replace current with upload** to update your SPLOT.
